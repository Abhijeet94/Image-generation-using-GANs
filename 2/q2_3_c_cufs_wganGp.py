...
def calc_gradient_penalty(netD, real_data, fake_data, batch_size):
    ...
    # gradient_penalty = ((gradients.norm(2, dim=1) - 1) ** 2).mean() * gradPen_lambda
    ...
    return gradPen_lambda * ((gradients_norm - 1) ** 2).mean()
...
for epoch in range(num_epochs):
    ...
    while i < len(dataloader):
        ...
        j = 0
        while j < d_iter and i < len(dataloader):
            j += 1
            data = data_iter.next()
            i += 1
            ...
            errD_real = output.mean() #criterion(output, label)
            ...
            errD_fake = -1.0 * output.mean() #criterion(output, label)
            # Calculate the gradients for this batch
            errD_fake.backward()
            D_G_z1 = output.mean().item()

            # train with gradient penalty
            gradient_penalty = -1.0 * calc_gradient_penalty(netD, real_cpu.data, fake.detach().data, b_size)
        ...
        if (iters % 500 == 0) or ((epoch == num_epochs-1) and (i-1 == len(dataloader)-1)):
            with torch.no_grad():
